            ...
            <nav class="view-tabs">
                @foreach (['timeline' => 'Timeline', 'feed' => 'Feed'] as $value => $label)
                    <a class="view-tab {{ $view === $value ? 'active' : '' }}"
                       href="{{ route('brands.workspace', ['brand' => $brand, 'year' => $year, 'month' => $month, 'type' => $type, 'view' => $value]) }}">{{ $label }}</a>
                @endforeach
            </nav>
            <nav class="filters">
                <a class="chip {{ $type === 'semua' ? 'active' : '' }}"
                   href="{{ route('brands.workspace', ['brand' => $brand, 'year' => $year, 'month' => $month, 'type' => 'semua', 'view' => $view]) }}">Semua</a>
                @foreach ($contentTypes as $contentType)
                    <a class="chip {{ $type === $contentType->slug ? 'active' : '' }}"
                       href="{{ route('brands.workspace', ['brand' => $brand, 'year' => $year, 'month' => $month, 'type' => $contentType->slug, 'view' => $view]) }}">{{ $contentType->name }}</a>
                @endforeach
            </nav>
